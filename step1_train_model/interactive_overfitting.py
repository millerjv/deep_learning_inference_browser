...
def interactive_overfitting(data_path, img_rows, img_cols, input_no  = 3, output_no = 3,
    ...
    axes[0, 2].imshow(msks_test[test_no].squeeze()[:,:,0])
    axes[0, 2].axis('off')
    axes[0, 3].axis('off')
    axes[0, 4].axis('off')

    axes[1, 0].set_title('Markup @{}'.format(1))
    axes[1, 0].imshow(modified_inference([msks_pred, 0])[0].squeeze()[:,:,0])
    axes[1, 0].axis('off')
    axes[1, 1].set_title('Diff-inference @{}'.format(1))
    axes[1, 1].imshow(belief([msks_pred, 0])[0].squeeze()[:,:,0])
    axes[1, 1].axis('off')
    axes[1, 2].set_title('Normalized gradients @{}'.format(1))
    im_grad = axes[1, 2].imshow(normalized_gradients([test, 0])[0].squeeze()) #, clim=clim_grad)
    axes[1, 2].axis('off')
    axes[1, 3].set_title('Perturbed input @{}'.format(1))
    axes[1, 3].imshow(perturbed_input_img[0].squeeze(), clim=clim)
    axes[1, 3].axis('off')
    axes[1, 4].set_title('GP inference @{}'.format(1))
    perturbed_msks_pred = model.predict(perturbed_input_img, batch_size=128, verbose=0)
    axes[1, 4].imshow(perturbed_msks_pred[0].squeeze()[:,:,0])
    axes[1, 4].axis('off')


    axes[2, 0].set_title('Markup @{}'.format(iter))
    axes[2, 0].imshow(modified_inference([last_msks_pred, 0])[0].squeeze()[:,:,0])
    axes[2, 0].axis('off')
    axes[2, 1].set_title('Diff-inference @{}'.format(iter))
    axes[2, 1].imshow(belief([last_msks_pred, 0])[0].squeeze()[:,:,0])
    axes[2, 1].axis('off')
    axes[2, 2].set_title('Normalized gradients@{}'.format(iter))
    im_grad = axes[2, 2].imshow(normalized_gradients([last_test, 0])[0].squeeze()) #, clim=clim_grad)
    axes[2, 2].axis('off')
    axes[2, 3].set_title('Perturbed input @{}'.format(iter))
    axes[2, 3].imshow(last_test.squeeze(), clim=clim)
    axes[2, 3].axis('off')
    axes[2, 4].set_title('GP inference @{}'.format(iter))
    axes[2, 4].imshow(last_msks_pred[0].squeeze()[:,:,0])
    axes[2, 4].axis('off')
